...
namespace NuPattern.Library.TypeEditors
{
	...
	/// </devdoc>
	internal class TemplateUriEditor : UITypeEditor
	{
		private const string VsTemplateExtension = ".vstemplate";
		/// <summary>
		...
		public override object EditValue(ITypeDescriptorContext context, IServiceProvider provider, object value)
		{
			...
			picker.Title = Properties.Resources.TemplateUriEditor_PickerTitle;
			picker.RootItem = solution;
			picker.EmptyItemsMessage = Resources.TemplateUriEditor_EmptyItemsMessage;
			//// TODO Should correct this query in order to get the selected item
			//picker.SelectedItem = solution.Items.Traverse(item => item.Items).FirstOrDefault(item => item.Name == templateUri);
			...
			picker.Filter.IncludeFileExtensions = VsTemplateExtension;
			...
			if (picker.ShowDialog().GetValueOrDefault())
			{
				...
				if (!ExtensionMatches(picker.SelectedItem) || picker.SelectedItem.Parent.Kind == ItemKind.Item)
				{
					...
					{
						messageService.ShowWarning(Resources.TemplateUriEditor_ReferToGuidance);
					}
				}
				else
				{
		...
		{
			return string.Equals(Path.GetExtension(item.PhysicalPath), VsTemplateExtension);
		}
