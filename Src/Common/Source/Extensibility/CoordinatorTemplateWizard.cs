	...
	public class CoordinatorTemplateWizard : TemplateWizard
	{
		...
		public override void RunStarted(object automationObject, Dictionary<string, string> replacementsDictionary, VisualStudio.TemplateWizard.WizardRunKind runKind, object[] customParams)
		{
			...
			if (wizardData != null)
			{
				...
				using (var services = new ServiceProvider((Ole.IServiceProvider)automationObject))
				{
					var components = services.GetService<SComponentModel, IComponentModel>();

					foreach (var wizard in wizards)
					{
						TryOrDispose(() => components.DefaultCompositionService.SatisfyImportsOnce(wizard));
					}
				}
			}
				...
				try
				{
					...
					try
					{
						...
						if (!typeof(IWizard).IsAssignableFrom(type))
							tracer.TraceError(Resources.CoordinatorTemplateWizard_NotIWizard, type.FullName);
						else
